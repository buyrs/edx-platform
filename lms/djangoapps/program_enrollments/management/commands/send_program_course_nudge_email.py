...
class Command(BaseCommand):
    ...
    def get_course_run_to_suggest(self, programs_progress):
        """
    ...
    def emit_event(self, user, course_run):
        """
        ...
        event_properties = {
            ...
            'course_url': urljoin(settings.MKTG_URLS.get('ROOT'), course_run['marketing_url']),
        ...
        LOGGER.info(
            '[Program Course Nudge Email] Segment event fired to suggest Course [%s] to User [%s].',
            event_properties['course_id'],
            event_properties['username'],
        )
    ...
    def handle(self, *args, **options):
        ...
        for course_id, users in course_to_users_maps.items():
            course_linked_programs = get_programs(course=course_id)
            ...
            if course_linked_programs:
                for user in users:
                    ...
                    course_run = self.get_course_run_to_suggest(programs_progress)
                    if course_run:
                        if should_commit:
                            self.emit_event(user, course_run)
